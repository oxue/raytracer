...
vec3 phongModel(vec3 fragPosition, vec3 fragNormal) {
    ...
    float selectedMultiplier = 1.0;

    if(isSelected){
        return (ambientIntensity + selectedMultiplier * light.rgbIntensity * (diffuse + specular)) * time + vec3(1.0,1.0,0.0) * (1-time);
    }

    return ambientIntensity + selectedMultiplier * light.rgbIntensity * (diffuse + specular);
}
