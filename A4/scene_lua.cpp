...
int gr_cyl_cmd(lua_State* L)
{
  ...
  const char* name = luaL_checkstring(L, 1);
  data->node = new GeometryNode(name, new Cyl());
...
int gr_material_cmd(lua_State* L)
{
  ...
  double kt = luaL_checknumber(L, 5);
  

  PhongMaterial* newMat = new PhongMaterial(glm::vec3(kd[0], kd[1], kd[2]),
  ...
  ((PhongMaterial*)data->material)->m_refractiveIndex = 1.6;
